  ...
  <div class="nupin">
    <div class="header_slider" >
      <ul class="ul_slider" ref="ul_slider">
        <li v-for="item in nupinSlideImage.concat(nupinSlideImage[0])" class="slider_li">
          <img :src=item alt="">
        </li>
      </ul>
      <div class="square">
        <ul class="square_ul"  ref="square_ul">
          <li class="square_li" v-for="(item,index) in nupinSlideImage" @click="scrollToSelectImg(index)"
    ...
    <img src="http://img62.ddimg.cn/www/a/8/3qie.jpg" alt="">
...
<script>
  ...
  import RedTextmodule from "../男频/redtext.vue"
  ...
  created:function () {
    let self = this
    this.$axios.get('/api/nupinContent')
      .then((res) => {
      res = res.data
    if (res.error === 0) {
